...
struct torus6p_init_params {
    torus6p_init_params() : x_dim(0), y_dim(0), no_vcs(0), credits(0), link_width(0), ni_up_credits(0), ni_upstream_buffer_size(0) {}
...
template <typename T>
Torus6p<T>::Torus6p(manifold::kernel::Clock& clk, torus6p_init_params* params, const Terminal_to_net_mapping* mapping, SimulatedLen<T>* slen, VnetAssign<T>* vn, int ni_credit_type, vector<int>* node_lp) :
    x_dim(params->x_dim),
    y_dim(params->y_dim),
    clk_t(clk),
    node_lp(node_lp)
{
    ...
    //open trace file
    outFile_data.open("torus6p_data.tr");
    ...
    //open trace file
    outFile_signal.open("torus6p_signal.tr");
    ...
    {
        for ( uint j=0; j< intf_per_router; j++)
          interface_ids.push_back( manifold::kernel::Component::Create< GenNetworkInterface<T> >(node_lp->at(i), i * intf_per_router + j, niInit, &i_p_inf) );
#ifdef IRIS_TEST
...
template <typename T>
void
Torus6p<T>::connect_interface_routers()
{
    int LATENCY = 1;
...
template <typename T>
void Torus6p<T> :: print_stats(std::ostream& out)
{
